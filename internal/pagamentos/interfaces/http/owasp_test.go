...
func TestOWASP_Admin_NoAuth_401(t *testing.T) {
	...
	for _, path := range []string{
		"/admin/pagamentos",
		"/admin/tenants/" + e.tenantID + "/pagamentos",
		"/admin/tenants/" + e.tenantID + "/pagamentos/novo",
	} {
...
func TestOWASP_SQLi_Status_NaoVaza(t *testing.T) {
	...
	w := httptest.NewRecorder()
	e.router.ServeHTTP(w, getReq("/admin/pagamentos?status=%27+OR+1%3D1+--", tokenCookie(token)))
	assert.Equal(t, http.StatusOK, w.Code)
}
...
func TestOWASP_SQLi_DataInicial_NaoVaza(t *testing.T) {
	...
	token := e.adminToken(t)
	w := httptest.NewRecorder()
	e.router.ServeHTTP(w, getReq("/admin/pagamentos?data_inicial=%27+OR+1%3D1+--", tokenCookie(token)))
	assert.Equal(t, http.StatusOK, w.Code)
}
...
func TestOWASP_SQLi_TenantID_NaoVaza(t *testing.T) {
	...
	token := e.adminToken(t)
	w := httptest.NewRecorder()
	e.router.ServeHTTP(w, getReq("/admin/pagamentos?tenant_id=%27%20OR%201%3D1%20--", tokenCookie(token)))
	assert.Equal(t, http.StatusOK, w.Code)
...
func TestOWASP_XSS_Descricao_Escapado(t *testing.T) {
	...
	e.router.ServeHTTP(w2, getReq("/admin/tenants/"+e.tenantID+"/pagamentos", tokenCookie(token)))
...
func TestOWASP_AdminPagar_InvalidID_404(t *testing.T) {
	...
	token := e.adminToken(t)
	w := httptest.NewRecorder()
	e.router.ServeHTTP(w, postForm("/admin/pagamentos/not-a-real-id/pagar", url.Values{}, tokenCookie(token)))
	assert.Equal(t, http.StatusNotFound, w.Code)
}
...
func TestOWASP_AdminCancelar_InvalidID_404(t *testing.T) {
	...
	token := e.adminToken(t)
	w := httptest.NewRecorder()
	e.router.ServeHTTP(w, postForm("/admin/pagamentos/deadbeef/cancelar", url.Values{"motivo": {"x"}}, tokenCookie(token)))
	assert.Equal(t, http.StatusNotFound, w.Code)
}
...
func TestOWASP_InvalidToken_401(t *testing.T) {
	e := setup(t)
	w := httptest.NewRecorder()
	e.router.ServeHTTP(w, getReq("/admin/pagamentos", &http.Cookie{Name: "token", Value: "invalid.token.here"}))
	assert.Equal(t, http.StatusUnauthorized, w.Code)
}
